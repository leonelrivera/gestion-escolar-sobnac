...
export default function CyclesPage() {
    ...
    const fetchCycles = async () => {
        setLoading(true);
        try {
            const res = await fetch('http://localhost:3001/cycles', {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = editing ? `http://localhost:3001/cycles/${editing.id}` : 'http://localhost:3001/cycles';
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`http://localhost:3001/cycles/${id}`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
            if (res.ok) {
