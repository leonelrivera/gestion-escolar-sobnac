...
export default function BoletinesPage() {
    ...
    const handleDownloadCourse = async () => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/boletines/course?cursoId=${selectedCurso}&cicloId=${selectedCiclo}`, {
            ...
            if (res.ok) {
                ...
                a.href = url;
                a.download = `boletines_curso.pdf`;
    ...
    const handleDownloadStudent = async (studentId: number, nombre: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/boletines/student/${studentId}?cursoId=${selectedCurso}&cicloId=${selectedCiclo}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.ok) {
                const blob = await res.blob();
                ...
                a.style.display = 'none';
                a.href = url;
                a.download = `boletin_${nombre.replace(/ /g, '_')}.pdf`;
                document.body.appendChild(a);
                a.click();
                setTimeout(() => {
                    document.body.removeChild(a);
        ...
        <div className="flex flex-col h-full space-y-6">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-2xl font-black text-black">Boletines de Evaluación Formativa</h1>
                </div>
                {selectedCurso && (
                    <button
                        onClick={handleDownloadCourse}
                        disabled={downloadingCourse}
                        className="px-6 py-3 bg-amber-600 text-white font-black rounded-xl hover:bg-amber-700 transition shadow-lg flex items-center gap-2 disabled:bg-amber-400"
                    >
                        {downloadingCourse ? 'Generando PDF...' : 'Descargar Curso Completo'}
                    </button>
                )}
            </div>

            <div className="bg-white p-5 rounded-xl shadow-sm border-2 border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                <div>
            ...
            <div className="bg-white rounded-xl shadow-sm border-2 border-gray-200 overflow-hidden flex-1">
                ...
                {loading ? (
                    ...
                    <ul className="divide-y-2 divide-gray-100 max-h-[500px] overflow-y-auto">
                        {students.map((st: any) => (
                            <li key={st.id} className="p-4 flex items-center justify-between hover:bg-amber-50 transition-colors">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 bg-amber-100 text-amber-700 border-2 border-amber-200 font-bold rounded-lg hover:bg-amber-600 hover:text-white transition disabled:opacity-50"
                                >
                                    {downloadingStudent === st.id ? 'Generando...' : 'Descargar Boletín'}
                                </button>
                            </li>
                        ))}
                    </ul>
