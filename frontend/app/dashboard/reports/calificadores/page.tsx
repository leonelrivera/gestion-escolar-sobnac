...
export default function CalificadoresPage() {
    ...
    const handleDownloadCourse = async () => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/calificadores/course?cursoId=${selectedCurso}`, {
            ...
            if (res.ok) {
                ...
                a.href = url;
                a.download = `calificadores_curso.pdf`;
    ...
    const handleDownloadStudent = async (studentId: number, nombre: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reports/calificadores/student/${studentId}?cursoId=${selectedCurso}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.ok) {
                const blob = await res.blob();
                ...
                a.style.display = 'none';
                a.href = url;
                a.download = `calificador_${nombre.replace(/ /g, '_')}.pdf`;
                document.body.appendChild(a);
                a.click();
                setTimeout(() => {
                    document.body.removeChild(a);
        ...
        <div className="flex flex-col h-full space-y-6">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-2xl font-black text-black">Calificadores Anuales</h1>
                </div>
                {selectedCurso && (
                    <button
                        onClick={handleDownloadCourse}
                        disabled={downloadingCourse}
                        className="px-6 py-3 bg-blue-600 text-white font-black rounded-xl hover:bg-blue-700 transition shadow-lg flex items-center gap-2 disabled:bg-blue-400"
                    >
            ...
            <div className="bg-white rounded-xl shadow-sm border-2 border-gray-200 overflow-hidden flex-1">
                ...
                {loading ? (
                    ...
                    <ul className="divide-y-2 divide-gray-100 max-h-[500px] overflow-y-auto">
                        {students.map((st: any) => (
                            <li key={st.id} className="p-4 flex items-center justify-between hover:bg-blue-50 transition-colors">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 bg-blue-100 text-blue-700 border-2 border-blue-200 font-bold rounded-lg hover:bg-blue-600 hover:text-white transition disabled:opacity-50"
                                >
